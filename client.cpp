...
int main(int argc, char* argv[]) {
	...
	if (client_socket == 1) {		
		cout << "La création du socket à bien été effectué" << endl;
	}
	...
	int reponse_write = write(client_socket, &ipkt, sizeof(init_packet)); 
	...
	int recu = write(client_socket, &msg, sizeof(msg));
	if (recu <= 0  ) {
